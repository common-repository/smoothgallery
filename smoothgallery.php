...
# Copyright (C) 2008-2014 Christian Schenk
# 
...
function smoothgallery_image_attachment_fields_to_edit($form_fields, $post) {
	if (substr($post->post_mime_type, 0, 5) == 'image') {
		# Extract SmoothGallery-Link from image meta
		$link = wp_get_attachment_metadata($post->ID);
		$link_form_value = null;
		if (is_array($link) and is_array($link['image_meta']) and isset($link['image_meta']['smoothgallery_link']))
			$link_form_value = $link['image_meta']['smoothgallery_link'];

		# Add SmoothGallery-Link to form fields
		$form_fields['smoothgallery_link'] = array('label' => __('SmoothGallery link', 'smoothgallery'),
		                                           'value' => $link_form_value,
